  ...
  return (
    <BrowserRouter>
      <div className="flex h-screen bg-gray-50">
        ...
        {state.user && ( // Only render sidebar if user is logged in
          <AppSidebar
            userId={state.user.uid}
            userName={state.user.displayName || 'User'}
            collapsed={isSidebarCollapsed} // Pass the collapsed state
            onToggleCollapse={handleSidebarToggle} // Pass the toggle function
          />
        )}

        {/* Main Content */}
        <main className={`flex-1 overflow-auto ${
          state.user ? (isSidebarCollapsed ? 'ml-16' : 'ml-5') : '' // Dynamically set margin
        }`}>
          <Routes>
            <Route
              path="/"
